...
CREATE TABLE `items` (
    ...
    `lastly_modified_by` bigint NOT NULL,
    ...
    KEY `idx_crew` (`lastly_modified_by`),
    KEY `idx_sale_state_order_count` (`sale_state`,`order_count` DESC),
    KEY `idx_order_count` (`order_count` DESC),
    KEY `idx_sale_price` (`sale_price`),
    KEY `idx_category_sale_price` (`category_id`,`sale_price`),
    KEY `idx_category_order_count` (`category_id`,`order_count` DESC),
    KEY `idx_store_order_count` (`store_id`,`order_count` DESC),
    KEY `idx_store_sale_price` (`store_id`,`sale_price`),
    KEY `idx_store_category_sale_price_sex_sale_state` (`store_id`,`category_id`,`sale_price`,`sex`,`sale_state`),
    KEY `idx_store_category_order_count_sex_sale_state` (`store_id`,`category_id`,`order_count` DESC),
...
CREATE TABLE orders (
    ...
    coupon_discounted_amount INT NOT NULL,
    payment_amount INT NOT NULL,
    orderer_id BIGINT NOT NULL,
    recipient_name VARCHAR(20) NOT NULL,
    road_address VARCHAR(100) NOT NULL,
    address_detail VARCHAR(255) NOT NULL,
    used_user_coupon_id BIGINT,
    used_coupon_name VARCHAR(100),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (item_id) REFERENCES items (id),
    FOREIGN KEY (item_option_id) REFERENCES item_options (id),
...
CREATE TABLE coupons (
    ...
    amounts BIGINT,
    discount_type ENUM('FIXED_DISCOUNT_KRW', 'RATE_DISCOUNT') NOT NULL,
